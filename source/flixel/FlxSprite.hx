...
class FlxSprite extends FlxObject
{
	...
	function drawSimple(camera:FlxCamera):Void
	{
		...
		if (Options.options == null)
			...
			camera.copyPixels(_frame, framePixels, _flashRect, _flashPoint, colorTransform, blend, antialiasing && Options.options.antialias);
	}
	...
	function drawComplex(camera:FlxCamera):Void
	{
		...
		if (Options.options == null)
			...
			camera.drawPixels(_frame, framePixels, _matrix, colorTransform, blend, antialiasing && Options.options.antialias, shader);
	}
	...
	public function stamp(Brush:FlxSprite, X:Int = 0, Y:Int = 0):Void
	{
		...
		else // complex render
		{
			...
			if (Options.options == null)
				...
				graphic.bitmap.draw(bitmapData, _matrix, null, brushBlend, null, Brush.antialiasing && Options.options.antialias);
		}
