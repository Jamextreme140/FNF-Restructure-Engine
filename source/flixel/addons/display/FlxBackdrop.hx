	...
	override public function draw():Void
	{
		...
		for (camera in cameras)
		{
			...
			if (FlxG.renderBlit)
			{
				...
				camera.copyPixels(frame, framePixels, _flashRect2, _ppoint, colorTransform, blend, antialiasing && Options.options.antialias);
			}
			else
			{
				if (_tileFrame == null)
				...
				var drawItem = camera.startQuadBatch(_tileFrame.parent, isColored, hasColorOffsets, blend, antialiasing && Options.options.antialias, shader);
